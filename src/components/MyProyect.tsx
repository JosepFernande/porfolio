...
type ProjectItemsMap = Record<string, ProjectItem>;

// Project data — text fields come from JSON via items[id]
...
const REGULAR_PROJECTS: ProjectData[] = [
    {
        id: "realtime-chat",
...
const PageHeader = ({ t }: { t: ReturnType<typeof useTranslations<"MyProjects">> }) => (
...
const FeaturedProject = ({ data, item, t }: { data: ProjectData; item: ProjectItem; t: ReturnType<typeof useTranslations<"MyProjects">> }) => (
    <section className="w-full">
        <h3 className="sr-only">{t("featured")}</h3>
...
const ProjectCard = ({ data, item, onClick, t }: { data: ProjectData; item: ProjectItem; onClick: () => void; t: ReturnType<typeof useTranslations<"MyProjects">> }) => (
    <article
...
const MoreProjectsCard = ({ t }: { t: ReturnType<typeof useTranslations<"MyProjects">> }) => (
...
const ProjectModal = ({ data, item, isOpen, onClose, t }: { data: ProjectData | null; item: ProjectItem | null; isOpen: boolean; onClose: () => void; t: ReturnType<typeof useTranslations<"MyProjects">> }) => {
    if (!isOpen || !item || !data) return null;

    const featureList = t.raw("modal.featureList") as string[];
                ...
                <div className="relative h-48 overflow-hidden">
                    ...
                    <div className="absolute bottom-6 left-6 right-6">
                        <div className="flex items-center gap-2 text-white/80 mb-2">
                            ...
                            <span className="text-xs font-bold uppercase tracking-widest">{t("modal.projectLabel")}</span>
                        </div>
...
export default function MyProyect() {
    ...
    // prettier-ignore
    const featuredItem = projectItems[FEATURED_PROJECT.id];
        ...
        <>
            <section aria-labelledby="proyectos-title" className="flex-1 flex flex-col items-center">
                <div className="w-full max-w-7xl flex flex-col gap-10">
                    <PageHeader t={t} />
                    <FeaturedProject data={FEATURED_PROJECT} item={featuredItem} t={t} />
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 auto-rows-fr">
                        ...
                        <MoreProjectsCard t={t} />
                    </div>
                </div>
            </section>

            <ProjectModal
                data={selectedProjectId ? [...REGULAR_PROJECTS, FEATURED_PROJECT].find(p => p.id === selectedProjectId) ?? null : null}
                item={selectedProjectId ? projectItems[selectedProjectId] : null}
                isOpen={isModalOpen}
